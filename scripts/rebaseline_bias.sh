...
#SBATCH --output=/opt/aar/eval-user/rebasebias_%j.out
R=/opt/aar/aar_repo
...
set -euo pipefail
echo "[rebasebias] $(date) key=$([ -n "${ANT_high_prio_API:-}${ANTHROPIC_API_KEY:-}" ] && echo set || echo MISSING)"
${PY} /opt/aar/eval-user/rebaseline_bias.py
echo "[rebasebias] DONE-WRAPPER"
